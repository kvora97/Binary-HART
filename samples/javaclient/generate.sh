...
COMMON_NAME="my_pi:123456789" openssl req -new -key certs/client/client.key -out certs/client/client.csr -config ssl.cnf -sha256
...
openssl pkcs12 -inkey certs/server/jetty.key -in certs/server/jetty.crt -export -out certs/server/jetty.pkcs12 -password pass:
